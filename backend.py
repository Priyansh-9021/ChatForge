...
llm=ChatGroq(api_key=api_key,model="openai/gpt-oss-20b", temperature=0.7,max_tokens=2000)
...
def get_stock_price(symbol: str) -> dict:
    ...
    """
    url = f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={symbol}&apikey=Y3W5CZZB0QZC5YCV"
    r = requests.get(url)
    return r.json()
...
def get_all_threads():
    all_threads = set()
    for ckpt in checkpoint_saver.list(None):
